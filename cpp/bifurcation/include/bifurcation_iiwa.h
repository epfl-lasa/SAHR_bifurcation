...
class Bifurcation {
	...
	private:
		...
		ros::Publisher _pubDesiredVelOrient;	// Publish desired velocities+orientation
		...
		std_msgs::Float64MultiArray _msgDesiredVelOrient;
		...
		Eigen::Vector4f _qd;
		Eigen::Matrix3f _wRb;
		Eigen::Vector3f _ax;		// axis angles
	...
	public:
		Bifurcation(ros::NodeHandle &n, Parameters *p, float frequency, int useMocap);
		int init();
		void run();
