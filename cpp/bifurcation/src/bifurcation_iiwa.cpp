...
Bifurcation::Bifurcation(ros::NodeHandle &n, Parameters *p, float frequency, int useMocap) :
	_n(n),
	p(p),
 	_loopRate(frequency)
{
...
int Bifurcation::init(){
  ...
  _pubDesiredVelOrient = _n.advertise<std_msgs::Float64MultiArray>("/iiwa/CustomControllers/command", 3);
  _msgDesiredVelOrient.data.resize(6);
...
void Bifurcation::run(){
	while (!_stop) {
  	if(!useMocap || (this->poseReceived && _firstObjectPose[0])) {
  		_mutex.lock();
...
void Bifurcation::publishData()
{
  ...
  for(int i=0; i<3; i++)
    _msgDesiredVelOrient.data[i] = this->vd[i];
  ...
  for(int i=0; i<3; i++){
    _msgDesiredVelOrient.data[i+3] = _ax(i);
  }

  // _pubDesiredTwist.publish(_msgDesiredTwist);
  // _pubDesiredOrientation.publish(_msgDesiredOrientation);
  _pubDesiredVelOrient.publish(_msgDesiredVelOrient);
}
...
void Bifurcation::getPosVel() {
  tf::StampedTransform transform;

  try{
    listener.lookupTransform("world", "iiwa_link_ee", ros::Time(0), transform);

    x(0) = transform.getOrigin().getX();
    x(1) = transform.getOrigin().getY();
    x(2) = transform.getOrigin().getZ();

    myfile << ros::Time::now().toSec() << ", " << x(0) << ", " << x(1) << ", " << x(2) << "\n";
    if(!this->poseReceived)
      this->poseReceived = true;

    auto tr = transform.getRotation();
    _q << tr.w(), tr.x(), tr.y(), tr.z();
    _wRb = Utils::quaternionToRotationMatrix(_q);
  }
...
void Bifurcation::computeDesiredOrientation()
{
  ...
  if((qtemp.segment(1,3)).norm() < 1e-3f) {
    _ax = qtemp.segment(1,3);
  }
  else {
    _ax = qtemp.segment(1,3)/(qtemp.segment(1,3)).norm();
  }
  angle = 2*std::acos(qtemp(0));
  _ax *= angle;
