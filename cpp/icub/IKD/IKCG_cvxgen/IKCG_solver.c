/* Produced by CVXGEN, 2019-04-30 09:43:34 -0400.  */
...
void IKCG_fillrhs_aff(void) {
  ...
  r1 = IKCG_work.rhs;
  r2 = IKCG_work.rhs + 74;
  r3 = IKCG_work.rhs + 138;
  r4 = IKCG_work.rhs + 202;
  /* r1 = -A^Ty - G^Tz - Px - q. */
...
void IKCG_better_start(void) {
  ...
  z = IKCG_work.lhs_aff + 138;
  y = IKCG_work.lhs_aff + 202;
...
long IKCG_solve(void) {
  ...
  for (iter = 0; iter < IKCG_settings.max_iters; iter++) {
    ...
    dz = IKCG_work.lhs_aff + 138;
    dy = IKCG_work.lhs_aff + 202;
    /* Find min(min(ds./s), min(dz./z)). */
    minval = 0;
    for (i = 0; i < 64; i++)
