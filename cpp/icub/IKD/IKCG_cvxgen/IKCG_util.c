...
float IKCG_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*IKCG_ran1(idum, reset)-1.0;
      v2 = 2.0*IKCG_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double IKCG_randn(void) {
  return IKCG_randn_internal(&IKCG_global_seed, 0);
}
void IKCG_reset_rand(void) {
  srand(15);
  IKCG_global_seed = 1;
  IKCG_randn_internal(&IKCG_global_seed, 1);
}
