...
float icub_id_randn_internal(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*icub_id_ran1(idum, reset)-1.0;
      v2 = 2.0*icub_id_ran1(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double icub_id_randn(void) {
  return icub_id_randn_internal(&icub_id_global_seed, 0);
}
void icub_id_reset_rand(void) {
  srand(15);
  icub_id_global_seed = 1;
  icub_id_randn_internal(&icub_id_global_seed, 1);
}
