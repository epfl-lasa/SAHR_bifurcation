...
void IKCG_setup_pointers(void) {
  IKCG_work.y = IKCG_work.x + 68;
  IKCG_work.s = IKCG_work.x + 98;
  IKCG_work.z = IKCG_work.x + 162;
  IKCG_vars.delta = IKCG_work.x + 0;
  IKCG_vars.dq = IKCG_work.x + 30;
}
...
void IKCG_fillrhs_aff(void) {
  ...
  r1 = IKCG_work.rhs;
  r2 = IKCG_work.rhs + 68;
  r3 = IKCG_work.rhs + 132;
  r4 = IKCG_work.rhs + 196;
  /* r1 = -A^Ty - G^Tz - Px - q. */
...
void IKCG_fillrhs_cc(void) {
  ...
  double minval;
  r2 = IKCG_work.rhs + 68;
  ds_aff = IKCG_work.lhs_aff + 68;
  dz_aff = IKCG_work.lhs_aff + 132;
  mu = 0;
  for (i = 0; i < 64; i++)
...
void IKCG_better_start(void) {
  ...
  x = IKCG_work.lhs_aff;
  s = IKCG_work.lhs_aff + 68;
  z = IKCG_work.lhs_aff + 132;
  y = IKCG_work.lhs_aff + 196;
...
long IKCG_solve(void) {
  ...
  for (iter = 0; iter < IKCG_settings.max_iters; iter++) {
    ...
    ds = IKCG_work.lhs_aff + 68;
    dz = IKCG_work.lhs_aff + 132;
    dy = IKCG_work.lhs_aff + 196;
    /* Find min(min(ds./s), min(dz./z)). */
    minval = 0;
    for (i = 0; i < 64; i++)
