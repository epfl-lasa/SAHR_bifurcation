...
void get_info(Contact_Manager &CM, Model &model, Cvector q0, Cmatrix &J, Cvector &X0, Cvector &Xr, Cvector &slack)
{
    ...
    for(int index=0;index<CM.size();index++)
    {
		...
		Cvector xact = quat_sub(model.get_orient_quat(body), CM.C[index].ref_o.pos);
		Cvector xref = ang2quat(Cvector3(0,0,0));

        X0.segment(beg_index,6)        = vectorbig( model.get_pos(body, offset) , xact.segment(0,3) );
        Xr.segment(beg_index,6)        = vectorbig( CM.C[index].ref_p.pos, 		  xref.segment(0,3) );

        slack.segment(beg_index,6)     = vectorbig( CM.C[index].T.slack_cost, 	CM.C[index].R.slack_cost );
		J.block(beg_index,0,6,AIR_N_U) = model.get_jacob(body, offset, CT_FULL);
