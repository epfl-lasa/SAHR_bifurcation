...
void Contact_Manager::initialize(Model * model, int size)
{
    M = model;
    C.resize(size);
	ifCoM = false;
...
{
    unsigned int M = 30;
    J = Cmatrix::Zero(M,AIR_N_U);
    dJdq = Cvector::Zero(M);
    acc = Cvector::Zero(M);
    slack = Cvector::Zero(M);
    int beg_index = 0;
    for(unsigned int i=0;i<C.size();i++)
    {
...
void Contact_Manager::control(Contact_Name index, Geom_Point ref_pos, Geom_Rot ref_ori, bool if_force)
{
	if(if_force)
	{
		...
		C[index].R.F_ref = C[index].R.K[0] * quat_sub(C[index].ref_o.pos, C[index].o.pos).segment(0,3)
